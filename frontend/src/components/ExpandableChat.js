...
const ExpandableChat = ({ isOpen, onClose, onRefresh, userData }) => {
  ...
  const generateKundli = async (detailsOverride) => {
    ...
    try {
      ...
      // For now: skip Kundli JSON generation; only fetch visual chart
      const successMessage = {
        id: nextMessageId(),
        text: `Bahut badhiya ${userProfile.name} ji! Main aapka visual chart generate kar raha hun...`,
        sender: 'pandit',
        timestamp: new Date().toLocaleTimeString()
      };
      setMessages(prev => [...prev, successMessage]);

      // Directly call chart endpoint
